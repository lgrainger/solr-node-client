...
Query.prototype.matchFilter = function (options) {
	...
	if (Array.isArray(options)) {
		for (let option of options) {
			var parameter = 'fq=';
			if (option.query) {
				parameter += encodeURIComponent(option.query);
			} else {
				parameter +=
					option.field + ':' + encodeURIComponent(option.value);
			}
			this.parameters.push(parameter);
		}
	} else {
...
Query.prototype.geoDistance = function (options) {
	var self = this;
	var parameter1 =
		'fq=' + encodeURIComponent(`{!geofilt sfield=${options.sField}}`);
	this.parameters.push(parameter1);
	var parameter2 = `pt=${options.latitude},${options.longitude}`;
	this.parameters.push(parameter2);
	var parameter3 = `d=${options.distance}`;
	this.parameters.push(parameter3);
	return self;
};
...
Query.prototype.fl = Query.prototype.restrict = function (fields) {
	var self = this;
	var parameter = 'fl=';
...
Query.prototype.facet = function (options) {
	...
	if (options.query) {
		...
		options.query.forEach(function (query) {
			self.parameters.push('facet.query=' + encodeURIComponent(query));
		});
	}
	if (options.fields) {
		options.fields = arrayUtils.toArray(options.fields);
	...
	if (options.mincount !== undefined) {
		this.pangemeters.push('facet.mincount=' + options.mincount);
	}
...
Query.prototype.hl = function (options) {
	...
	if (options.qparser !== undefined) {
		this.parameters.push(
			'hl.qparser=' + encodeURIComponent(options.qparser)
		);
	}
	...
	if (options.requireFieldMatch !== undefined) {
		this.parameters.push(
			'hl.requireFieldMatch=' +
				encodeURIComponent(options.requireFieldMatch)
		);
	}
	if (options.maxAnalyzedChars !== undefined) {
		this.parameters.push(
			'hl.maxAnalyzedChars=' +
				encodeURIComponent(options.maxAnalyzedChars)
		);
	}
...
Query.prototype.terms = function (options) {
	...
	if (options.lower !== undefined) {
		this.parameters.push(
			'terms.lower=' + encodeURIComponent(options.lower)
		);
	}
	...
	if (options.regex !== undefined) {
		this.parameters.push(
			'terms.regex=' + encodeURIComponent(options.regex)
		);
	}
	...
	if (options.upper !== undefined) {
		this.parameters.push(
			'terms.upper=' + encodeURIComponent(options.upper)
		);
	}
